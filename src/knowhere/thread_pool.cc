...
namespace knowhere {
    ...
    ThreadPool::InitGlobalBuildThreadPool(uint32_t num_threads) {
        ...
        if (build_pool_ == nullptr) {
            ...
            if (build_pool_ == nullptr) {
                ...
                LOG_INFO("Init global build thread pool with size %d", num_threads);
                return;
            }
        } else {
            LOG_INFO("Global build thread pool size has already been initialized to %d", build_pool_->size());
        }
    }
    ...
    ThreadPool::InitGlobalSearchThreadPool(uint32_t num_threads) {
        ...
        if (search_pool_ == nullptr) {
            ...
            if (search_pool_ == nullptr) {
                ...
                LOG_INFO("Init global search thread pool with size %d", num_threads);
                return;
            }
        } else {
            LOG_INFO("Global search thread pool size has already been initialized to %d", search_pool_->size());
        }
    }
    ...
    ThreadPool::SetGlobalBuildThreadPoolSize(uint32_t num_threads) {
        if (build_pool_ == nullptr) {
            ...
            LOG_INFO("Global build thread pool size has already been set to %d", build_pool_->size());
            return;
        }
    }
    ...
    ThreadPool::SetGlobalSearchThreadPoolSize(uint32_t num_threads) {
        if (search_pool_ == nullptr) {
            ...
            LOG_INFO("Global search thread pool size has already been set to %d", search_pool_->size());
            return;
        }
    }
